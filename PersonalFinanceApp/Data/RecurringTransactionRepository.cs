    ...
    public class RecurringTransactionRepository
    {
            ...
            using (var conn = DatabaseHelper.GetConnection())
            {
                ...
                string query = @"
                    SELECT r.recurring_id, r.user_id, r.category_id, r.amount, r.type, r.description,
                           r.is_active, r.frequency, r.last_processed_month, r.last_processed_year,
                           r.last_processed_date, r.created_at,
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            list.Add(new RecurringTransaction
                            {
                                ...
                                LastProcessedDate = reader.IsDBNull(reader.GetOrdinal("last_processed_date")) ? null : reader.GetDateTime(reader.GetOrdinal("last_processed_date")),
        ...
        public int Add(RecurringTransaction r)
        {
            using (var conn = DatabaseHelper.GetConnection())
            {
                conn.Open();
                string query = @"
                    INSERT INTO recurring_transactions (user_id, category_id, amount, type, description, is_active, frequency, created_at)
                    VALUES (@userId, @categoryId, @amount, @type, @description, TRUE, @frequency, @createdAt)
        ...
        public void UpdateLastProcessedDate(int recurringId, int userId, DateTime date)
        {
            using (var conn = DatabaseHelper.GetConnection())
            {
                conn.Open();
                string query = "UPDATE recurring_transactions SET last_processed_date = @date WHERE recurring_id = @recurringId AND user_id = @userId";

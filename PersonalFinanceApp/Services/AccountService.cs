...
namespace PersonalFinanceApp.Services
{
    public class AccountService
    {
        ...
        public decimal GetInvestBalance(int userId)
        {
            return _repository.GetInvestBalance(userId);
        }

        public void AdjustInvestBalance(int userId, decimal delta)
        {
            _repository.AdjustInvestBalance(userId, delta);
        }

        public bool TransferWalletToInvest(int userId, decimal amount, out string errorMessage)
        {
            errorMessage = string.Empty;
            var (wallet, _) = _repository.GetBalances(userId);

            if (amount <= 0) { errorMessage = "Tutar 0'dan büyük olmalıdır."; return false; }
            if (amount > wallet) { errorMessage = "Cüzdanınızda yeterli bakiye yok."; return false; }

            _repository.AdjustWalletBalance(userId, -amount);
            _repository.AdjustInvestBalance(userId, amount);
            _repository.LogTransfer(userId, "wallet_to_invest", amount);
            return true;
        }

        public bool TransferSafeToInvest(int userId, decimal amount, out string errorMessage)
        {
            errorMessage = string.Empty;
            var (_, safe) = _repository.GetBalances(userId);

            if (amount <= 0) { errorMessage = "Tutar 0'dan büyük olmalıdır."; return false; }
            if (amount > safe) { errorMessage = "Kasanızda yeterli bakiye yok."; return false; }

            _repository.AdjustSafeBalance(userId, -amount);
            _repository.AdjustInvestBalance(userId, amount);
            _repository.LogTransfer(userId, "safe_to_invest", amount);
            return true;
        }

        public bool TransferInvestToWallet(int userId, decimal amount, out string errorMessage)
        {
            errorMessage = string.Empty;
            decimal invest = _repository.GetInvestBalance(userId);

            if (amount <= 0) { errorMessage = "Tutar 0'dan büyük olmalıdır."; return false; }
            if (amount > invest) { errorMessage = "Yatırım bakiyenizde yeterli bakiye yok."; return false; }

            _repository.AdjustInvestBalance(userId, -amount);
            _repository.AdjustWalletBalance(userId, amount);
            _repository.LogTransfer(userId, "invest_to_wallet", amount);
            return true;
        }

        public bool TransferInvestToSafe(int userId, decimal amount, out string errorMessage)
        {
            errorMessage = string.Empty;
            decimal invest = _repository.GetInvestBalance(userId);
            ...
            _repository.AdjustInvestBalance(userId, -amount);
            _repository.AdjustSafeBalance(userId, amount);
            _repository.LogTransfer(userId, "invest_to_safe", amount);
            return true;
        }
